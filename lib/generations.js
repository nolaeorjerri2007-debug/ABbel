...
// 净化 scores：只保留纯数值键，剔除 draft/title 等字符串脏数据；无有效数值返回 null。
...
// 判断 Dify 返回是否包含有效产出（生成 / 调音共用）。空输出视为失败，供退费判断。
export function hasValidOutput(mode, difyData, tunePayload) {
  const extracted = mode === 'tune'
    ? extractTune(difyData, tunePayload)
    : extractGenerate(difyData)
  return typeof extracted.output_draft === 'string' && extracted.output_draft.trim().length > 0
}
