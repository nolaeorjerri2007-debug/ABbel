...
// 退款：上游生成失败时，把已扣的 1 次额度原子加回。
// 仅在 deductQuota 成功后调用（此时 balance key 必然存在），单条 INCR 天然原子。
export async function refundQuota(redis, userId) {
  const balance = await redis.incr(userId);
  return { balance };
}
